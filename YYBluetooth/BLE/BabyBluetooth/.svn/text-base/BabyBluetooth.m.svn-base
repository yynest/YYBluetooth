...
-(void)start:(CBPeripheral *)cachedPeripheral{
    ...
    if (CENTRAL_MANAGER_INIT_WAIT_TIMES >=5 ) {
        DLog(@">>> 第%d次等待CBCentralManager 打开任然失败，请检查你蓝牙使用权限或检查设备问题。",CENTRAL_MANAGER_INIT_WAIT_TIMES);
        return;
    ...
    DLog(@">>> 第%d次等待CBCentralManager打开",CENTRAL_MANAGER_INIT_WAIT_TIMES);
}
...
//xiangfeng 蓝牙的状态
- (CBManagerState)getBuletoothCentralManagerstate {
    return babyCentralManager->centralManager.state;
}
